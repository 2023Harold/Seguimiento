...
<div>
        <p style="font-family: Arial; color: #000000; font-size: 14px; text-align: justify;">
            <strong>V I S T O </strong>el memorándum número <span style="color: blue;">{{$modelo->num_memo_recepcion_expediente}}</span>, presentado por Jaime Enrique Perdigón Nieto, Auditor Especial de Cumplimiento Financiero e Inversión Física del Órgano Superior de Fiscalización del Estado de México, por medio del cual remite a la Unidad de Seguimiento, el Expediente Técnico derivado de la Auditoría de Cumplimiento Financiero practicada a <span style="color: blue;">{{$relacion4['entidad']}}</span>,
            por el período comprendido <span style="color: blue;">{{ $auditoria->periodo_revision}}</span>; por lo que, con fundamento en los artículos 1, 3, 4 <span style="color: blue;">{{$relacion4['fraccion']}}</span>, 5, 6, 7, 8, 9, 21, 42 Bis, 53 fracción I, 54, <span style="color: blue;">{{$relacion4['recomendaciones01']}}</span> 55 párrafo segundo de la Ley de Fiscalización Superior del Estado de México y; 3 fracciones XIII Bis <span style="color: blue;">{{$relacion4['recomendaciones02']}}</span>
            , 4, 6 fracciones III, XVIII, <span style="color: blue;">{{$relacion4['siPRAS']}}</span> y XXXVII, 23 y 47 del Reglamento Interior del Órgano Superior de Fiscalización del Estado de México; se emite el siguiente:<br><br>
        </p>        
        <p style="font-family: Arial; color: #000000; font-size: 14px; text-align: center; width:100%;"><strong>ACUERDO</strong></p><br>
        <p style="font-family: Arial; color: #000000; font-size: 14px; text-align: justify;">
            <strong>PRIMERO.</strong> Se tienen por presentados el memorándum y el Expediente Técnico de la Auditoría de Cumplimiento Financiero, practicada <span style="color: blue;">{{$relacion4['entidad']}}</span>, por el período comprendido del <span style="color: blue;">{{ $auditoria->periodo_revision}}</span> y ordenada mediante oficio número <span style="color: blue;">{{$modelo->numero_acuerdo}}</span>; 
            por lo que <strong>se admiten</strong> para los trámites subsecuentes.<br><br>
            ...
            <span style="color: blue;"><strong> {{$relacion4['siPRAS01']}}</strong></span><br><br>
            ...
            para que comparezca de manera personal por sí o a través de su representante legal o enlace administrativo debidamente autorizado a las span style="color: blue;">{{strtolower($relacion4['hora01'])}}</span> horas<span style="color: blue;">, del día </span> <span style="color: blue;">{{strtolower($relacion4['day01'])}} de {{strtolower($relacion4['mes01'])}}</span>; en el domicilio de la Unidad de Seguimiento de este Órgano Superior de Fiscalización del Estado de México, sito en Avenida José María Pino Suárez Sur,
            números 104, 106 y 108, Colonia Cinco de Mayo, Toluca, Estado de México, C.P. 50090. Lo anterior con el objeto <strong>de que se puntualicen las observaciones detalladas en el Informe de Auditoría y se ponga a la vista del compareciente el Expediente Técnico de la Auditoría citada en el acuerdo PRIMERO del presente.</strong> <br><br>
            ...
            <strong>{{$relacion4['orden01']}} Se ordena el inicio de la Etapa de Aclaración <span style="color: blue;">{{$relacion4['recomendaciones03']}}</span> de las observaciones subsistentes en materia de Cumplimiento Financiero</strong> y, que se encuentran detalladas en el Informe de Auditoría; por lo cual, con fundamento en lo dispuesto en el artículo 54 fracción I de la Ley de Fiscalización Superior del Estado de México, <strong>se concede a la entidad fiscalizada un plazo de 30 (Treinta) días hábiles contados a partir del día 
            <span style="color: blue;">{{strtolower($relacion4['day02'])}} de {{strtolower($relacion4['mes02'])}}</span>; y que fenece el <span style="color: blue;">{{strtolower($relacion4['day03'])}} de {{strtolower($relacion4['mes03'])}}</span></strong>, a efecto de que se presenten los elementos, documentos y datos fehacientes que aclaren o solventen el contenido de las acciones de cuenta, o en su caso, manifieste lo que a su derecho convenga <span style="color: blue;">{{$relacion4['recomendaciones04']}}</span>.<br><br>
