        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                @if(count($auditoria->cedulageneralpras)>0)  
                    ...
                    @if(count($analistasF)>0 && count($analistasL)== 0 && empty($auditoria->cedulageneralpras[0]->fase_autorizacion))                           
                        {!! BootForm::open(['model' => $auditoria, 'update' => 'cedulageneralpras.update','id'=>'form']) !!}            
                        <div class="row">
                            <div class="col-md-12">                         
                                {!! BootForm::hidden('cedula2',$nombre)!!}                            
                    ...
                    @elseif(in_array(auth()->user()->id, $analistasF) && count($analistasL)>0)                    
                        <a href="{{ route('cedulageneralpraslider.edit',$auditoria->cedulageneralpras[0]) }}" class="btn btn-primary popuprevisar float-end">
                    ...
                    @if ($auditoria->cedulageneralpras[0]->fase_autorizacion == 'En revisión 01')                    
                        @can('cedulageneralprasrevision01.edit') 
                            @if(in_array(auth()->user()->id, $lideresF))
                                <a href="{{ route('cedulageneralprasrevision01.edit',$auditoria->cedulageneralpras[0]) }}" class="btn btn-primary popuprevisar float-end">
                                    <li class="fa fa-gavel"></li>
                                    Revisar
                                </a><br><br><br>
                            @endif
                    ...
                    @if ($auditoria->cedulageneralpras[0]->fase_autorizacion == 'En revisión')                    
                        @can('cedulageneralprasrevision.edit')
                            @if(in_array(auth()->user()->unidad_administrativa_id, $jefesF))
                                <a href="{{ route('cedulageneralprasrevision.edit',$auditoria->cedulageneralpras[0]) }}" class="btn btn-primary popuprevisar float-end">
                                    <li class="fa fa-gavel"></li>
                                    Revisar
                                </a><br><br><br>
                            @endif
                        @else
                            <span class="alert alert-warning float-end">{{ $auditoria->cedulageneralpras[0]->fase_autorizacion}} </span><br><br>
                        @endcan
                    @endif
                    @if ($auditoria->cedulageneralpras[0]->fase_autorizacion == 'En validación')
                        @can('cedulageneralprasvalidacion.edit')
                            <a href="{{ route('cedulageneralprasvalidacion.edit',$auditoria->cedulageneralpras[0]) }}" class="btn btn-primary popuprevisar float-end">
                                <li class="fa fa-gavel"></li>
                                Validar
                            </a><br><br><br>
                        @else
                            <span class="badge badge-light-warning float-end">{{ $auditoria->cedulageneralpras[0]->fase_autorizacion}} </span><br><br>
                        @endcan
                    @endif
                    @if ($auditoria->cedulageneralpras[0]->fase_autorizacion == 'En autorización')
                        @can('cedulageneralprasautorizacion.edit')
                            <a href="{{ route('cedulageneralprasautorizacion.edit',$auditoria->cedulageneralpras[0]) }}" class="btn btn-primary popuprevisar float-end">
                                <li class="fa fa-gavel"></li>
                                Autorizar
                            </a><br><br><br>
                        @else
                    ...
                    @if ($auditoria->cedulageneralpras[0]->fase_autorizacion=='Autorizado')
                        <span class="alert alert-success float-end">{{ $auditoria->cedulageneralpras[0]->fase_autorizacion }} </span><br><br>
                    @endif
                @else   
                    {!! BootForm::open(['model' => $auditoria, 'update' => 'cedulageneralpras.update','id'=>'form']) !!}  
                    {!! BootForm::hidden('cedula2',$nombre)!!}             
                ...
                <div class="table-responsive">
                    <table class="table">                        
                        <tbody>     							
                            {!! movimientosDesglose($auditoria->id, 1, $auditoria->movimientosCedulaPRAS) !!}                            
                        </tbody>
                    </table>
				</div>           
            </div>
