@extends('layouts.app')
@section('breadcrums')    
    {{Breadcrumbs::render('asignacionlideranalistaextra.edit',$auditoria) }}      
@endsection
@section('content')
<div class="card">
    <div class="card-header">
        <h1 class="card-title">
            <a href="{{ route('asignacionlideranalistaextra.index') }}"><i class="fa fa-arrow-alt-circle-left fa-1x text-primary"></i></a>            
            &nbsp; {{ $accions }}       
        </h1>
    </div>
    <div class="card-body">
        ...
        {!!BootForm::open(['model' => $auditoriaUsuarios,'update' => 'asignacionlideranalistaextra.update','store' => 'asignacionlideranalistaextra.store','id' => 'form']) !!}
        {!!BootForm::hidden('accions',$accions) !!}
        <!-- !! BootForm::hidden('usuario_id',null,['id'=>'usuario_id']) !! -->
        <div class="row">
            <div class="col-md-3">
                {!!BootForm::select('analista_id', 'Analista adicional: *',$analistas , old('analista_id',$auditoriaUsuarios->analista_id), ['data-control'=>'select2', 'class'=>'form-select', 'data-placeholder'=>'Seleccionar una opción']) !!}
            </div>
        </div> 
        ...
        <div class="row">
            <div class="col-md-6">         
                    @can('asignacionlideranalistaextra.update')                     
                        <button type="submit" class="btn btn-primary">Guardar</button>
                    @endcan  
                    <a href="{{ route('asignacionlideranalistaextra.index') }}" class="btn btn-secondary me-2">Cancelar</a>                
            </div>
        </div>
        {!!BootForm::close() !!}
    </div>
...
<script>
    $(document).ready(function() {
        ...
        function cargarCargo(userId) {
            if (userId) {
                $.ajax({
                    url: "{{ route('asignacionlideranalistaextra.getAna') }}",
                    dataType: "JSON",
                    type: "POST",
                    data: {
                        "user_id": userId,
...
{!!JsValidator::formRequest('App\Http\Requests\AsignacionAnalistaExtraRequest') !!}
@endsection
