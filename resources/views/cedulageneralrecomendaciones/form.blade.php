...
{{-- @section('breadcrums')
{{ Breadcrumbs::render('cedulageneralrecomendacion.edit',$auditoria) }}
@endsection --}}
@section('content')
<div class="row">
        ...
        <div class="card">
            <div class="card-header">
                <h1 class="card-title">
                    ...
                    &nbsp;
                    Cédula General Recomendaciones
                </h1>
            </div>
            <div class="card-body">
                ...
                @if(count($auditoria->cedulageneralrecomendaciones)>0)  
                    @if ($auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion == 'Rechazado')
                        ...
                        @endcan
                    @endif 
                    @if(count($analistasF)>0 && count($analistasL)== 0 && empty($auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion))                           
                        {!! BootForm::open(['model' => $auditoria, 'update' => 'cedulageneralrecomendacion.update','id'=>'form']) !!}            
                        <div class="row">
                            <div class="col-md-12">                         
                                {!! BootForm::hidden('cedula2',$nombre)!!}                            
                    ...
                    @elseif(in_array(auth()->user()->id, $analistasF) && count($analistasL)>0)                    
                        <a href="{{ route('cedgralrecomendacionanalista.edit',$auditoria->cedulageneralrecomendaciones[0]) }}" class="btn btn-primary popuprevisar float-end">
                    ...
                    @if ($auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion == 'En revisión 01')          
                        @can('cedgralrecomendacionrevision01.edit') 
                            @if(in_array(auth()->user()->id, $lideresF))
                                <a href="{{ route('cedgralrecomendacionrevision01.edit',$auditoria->cedulageneralrecomendaciones[0]) }}" class="btn btn-primary popuprevisar float-end">
                                    <li class="fa fa-gavel"></li>
                                    Revisar
                                </a><br><br><br>
                            @endif
                    ...
                    @if ($auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion == 'En revisión')     
                        @can('cedgralrecomendacionrevision.edit')
                            @if(in_array(auth()->user()->unidad_administrativa_id, $jefesF))
                                <a href="{{ route('cedgralrecomendacionrevision.edit',$auditoria->cedulageneralrecomendaciones[0]) }}" class="btn btn-primary popuprevisar float-end">
                                    <li class="fa fa-gavel"></li>
                                    Revisar
                                </a><br><br><br>
                            @endif
                        @else
                            <span class="alert alert-warning float-end">{{ $auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion}} </span><br><br>
                        @endcan
                    @endif
                    @if ($auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion == 'En validación')
                        @can('cedgralrecomendacionvalidacion.edit')
                            <a href="{{ route('cedgralrecomendacionvalidacion.edit',$auditoria->cedulageneralrecomendaciones[0]) }}" class="btn btn-primary popuprevisar float-end">
                                <li class="fa fa-gavel"></li>
                                Validar
                            </a><br><br><br>
                        @else
                            <span class="badge badge-light-warning float-end">{{ $auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion}} </span><br><br>
                        @endcan
                    @endif
                    @if ($auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion == 'En autorización')
                        @can('cedgralrecomendacionautorizacion.edit')
                            <a href="{{ route('cedgralrecomendacionautorizacion.edit',$auditoria->cedulageneralrecomendaciones[0]) }}" class="btn btn-primary popuprevisar float-end">
                                <li class="fa fa-gavel"></li>
                                Autorizar
                            </a><br><br><br>
                        @else
                    ...
                    @if ($auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion=='Autorizado')
                        <span class="alert alert-success float-end">{{ $auditoria->cedulageneralrecomendaciones[0]->fase_autorizacion }} </span><br><br>
                    @endif
                @else   
                    {!! BootForm::open(['model' => $auditoria, 'update' => 'cedulageneralrecomendacion.update','id'=>'form']) !!}  
                    {!! BootForm::hidden('cedula2',$nombre)!!}             
                ...
                <div class="table-responsive">
                    <table class="table">                        
                        <tbody>     							
                            {!! movimientosDesglose($auditoria->id, 1, $auditoria->movimientosCedulaRecomendacion) !!}                            
                        </tbody>
                    </table>
				</div>
